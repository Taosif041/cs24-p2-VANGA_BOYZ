...
authController.initiatePasswordReset = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'An error occurred while initiating password reset', error: error.toString() });
  }
};

authController.confirmPasswordReset = async (req, res) => {
  try {
    const passwordReset = await PasswordReset.findOne({ email: req.body.email, otp: req.body.otp });
    if (!passwordReset) {
    ...
    res.status(500).json({ message: 'An error occurred while confirming password reset', error: error.toString() });
  }
};
...
authController.login = async (req, res) => {
  try {
    ...
    if (user.roles.length === 0) {
      return res.status(403).json({ message: 'Unassigned user cannot enter the system. Wait for assignment.' });
    }
